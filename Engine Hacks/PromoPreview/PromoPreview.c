...
void DrawPromoPreview(u16 item) {
    ...
    struct CharBasedPromoListEntry* charPromoList = &CharBasedPromoList[0];
    while (charPromoList->baseClass != 0) {
        if (activeUnit.pCharacterData->number == charPromoList->charID) {
            if (activeUnit.pClassData->number == charPromoList->baseClass) break;
        }
    charPromoList ++;
    }
    if (charPromoList->baseClass == 0) {
	...
	u8 promotedClass = charPromoList->promoClass;
